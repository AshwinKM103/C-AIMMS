...
| **python-best-practices skill**  | Code style checks for new bridge class                     | ✅ Installed | `.claude/skills/python-best-practices/` | Use when writing `fluxmem/memocr_episodes.py`                  |
...
Agent(
  ...
  task="Design MemOCREpisodeProducer class (adapter for fluxmem). Take memory_img_final_only_triple.py and extract:
  1. Markdown memory state (self.memory, decoded per-batch-item text)
  2. Render call (md2img / call_md_renderer.batch_generate_images)
  3. Mapping into EpisodicUnit(primary_format=MemoryFormat.VC, ...)

  Map these to the fluxmem.interfaces.EpisodeProducer Protocol: produce(count) -> list[EpisodicUnit]
  fluxmem has no retriever/MemoryStore -- do not design against one; see fluxmem/ltsm.py's
  docstring (retrieval/ITERRET explicitly out of scope).
...
Agent(
  ...
  task="Write ADR: 'MemOCR/fluxmem integration: an EpisodeProducer adapter, not a retriever'. Format: Nygard ADR (Context, Decision, Consequences). Key points:
  - fluxmem has no retriever/MemoryStore -- retrieval (ITERRET) is explicitly out of scope (fluxmem/ltsm.py)
  - MemOCR provides visual-priority memory (markdown → image), a real encoder for MemoryFormat.VC
  - Bridge via EpisodeProducer adapter (MemOCREpisodeProducer in fluxmem/memocr_episodes.py)
  - Tradeoff: vision encoder/render cost vs. information density; visual_salience is an uncited modeling choice

  Store as docs/adr/0003-memocr-fluxmem-integration.md"
)
```
...
- After writing `fluxmem/memocr_episodes.py`
- Type hints, imports, naming conventions
- Consistency with `fluxmem/` codebase patterns (see `fluxmem/interfaces.py`, `fluxmem/stim.py`)
...
4. **Optional: archive variants** → Move non-paper memory\_\*.py to archived/
...
1. **Run refactoring-specialist** → Design `EpisodeProducer` adapter
2. **Write `fluxmem/memocr_episodes.py`** → Implement `MemOCREpisodeProducer`
3. **No factory to update** → `fluxmem/` has no factory/registry pattern; construct directly (see `fluxmem/interfaces.py`'s `StubEpisodeProducer` for the pattern)
4. **Test imports** → Verify no circular dependencies, module importable without torch/verl
5. **Run python-best-practices** → Style check new code

**Output:** Working `EpisodeProducer` adapter, integration tested
...
2. **Update CLAUDE.md** → Add MemOCR `EpisodeProducer` routing entry (not a retriever entry)
3. **Commit integration** → `git commit -m "feat(fluxmem): add MemOCR-backed EpisodeProducer for visual-format episodes"`
...
4. ✅ Implemented `MemOCREpisodeProducer` bridge class (`fluxmem/memocr_episodes.py`, implements `EpisodeProducer` — not a retriever)
